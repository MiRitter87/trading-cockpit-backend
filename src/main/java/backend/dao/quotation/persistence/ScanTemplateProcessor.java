...
public class ScanTemplateProcessor {
	...
	/**
	 * Performs post processing tasks for the given quotations based on the ScanTemplate "THREE_WEEKS_TIGHT".
	 * The method checks for each Instrument of the given quotations if the Instrument has closed three weeks in a row within a tight range.
	 * Those quotations that do not match the template are removed from the List of quotations.
	 * 
	 * @param quotations The quotations on which the post processing is performed.
	 * @throws Exception Post processing failed.
	 */
	private void postProcessingThreeWeeksTight(List<Quotation> quotations) throws Exception {
		...
		while(quotationIterator.hasNext()) {
			...
			quotationArray = new QuotationArray(this.quotationHibernateDAO.getQuotationsOfInstrument(currentQuotation.getInstrument().getId()));
	 ...
	 * @return true, if last three quotations closes within a tight range; false if not.
	 */
	private boolean isThreeWeeksTight(final List<Quotation> weeklyQuotations) {
		double upperBound = 0, lowerBound = 0;
		Quotation currentQuotation;
		
		if(weeklyQuotations.size() < 3)
			return false;
		...
		for(int i = 0; i < 3; i++) {
			...
			if(i == 0) {
				...
				if(currentQuotation.getClose().doubleValue() < lowerBound || currentQuotation.getClose().doubleValue() > upperBound)
					return false;
			}
		}
