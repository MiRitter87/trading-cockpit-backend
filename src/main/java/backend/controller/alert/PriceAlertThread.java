...
public class PriceAlertThread extends DataRetrievalThread {
     ...
     */
    public PriceAlertThread(final LocalTime startTime, final LocalTime endTime,
            final Map<StockExchange, DataProvider> dataProviders) throws Exception {
    ...
    private void checkAndUpdatePriceAlert(final PriceAlert priceAlert, final Quotation quotation) throws Exception {
        ...
        // If the trigger price has been reached, set the trigger time of the price
        // alert. Send Mail if requested.
    ...
    private String getMailSubject(final PriceAlert priceAlert) {
        ...
        if (priceAlert.getAlertType() == PriceAlertType.GREATER_OR_EQUAL) {
            subject = MessageFormat.format(this.resources.getString("priceAlert.mailSubjectGreatherThen"),
                    priceAlert.getInstrument().getSymbol(), priceAlert.getPrice(), priceAlert.getCurrency());
        } else if (priceAlert.getAlertType() == PriceAlertType.LESS_OR_EQUAL) {
            subject = MessageFormat.format(this.resources.getString("priceAlert.mailSubjectLessThen"),
                    priceAlert.getInstrument().getSymbol(), priceAlert.getPrice(), priceAlert.getCurrency());
        }
    ...
    private String getMailBody(final PriceAlert priceAlert, final Quotation quotation) {
        ...
        body = MessageFormat.format(this.resources.getString("priceAlert.mailBody"), formattedDate,
                quotation.getClose(), quotation.getCurrency(), priceAlert.getTriggerDistancePercent());
