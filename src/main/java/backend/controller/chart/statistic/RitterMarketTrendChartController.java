...
public class RitterMarketTrendChartController extends StatisticChartController {
    ...
    public JFreeChart getRitterMarketTrendChart() throws Exception {
        XYDataset dataset = this.getRitterMarketTrendDataset();
     ...
     */
    private XYDataset getRitterMarketTrendDataset() {
        Statistic statistic;
        ...
        for (int i = this.getStatistics().size() - 1; i >= 0; i--) {
            try {
                statistic = this.getStatistics().get(i);
                movingAverage = this.getMovingAverageOfRitterMarketTrend(periodOfMovingAverage, i);
     ...
     */
    private float getMovingAverageOfRitterMarketTrend(final int period, final int beginIndex) throws Exception {
        ...
        lastValidBeginIndex = this.getStatistics().size() - numberAdditionalDaysForSmaVolume - period;
