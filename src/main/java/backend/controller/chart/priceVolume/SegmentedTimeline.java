...
/**
 * A {@link Timeline} that implements a "segmented" timeline with included, excluded and exception segments.
 * <P>
 * A Timeline will present a series of values to be used for an axis. Each Timeline must provide transformation methods
 * between domain values and timeline values.
 * <P>
 * A timeline can be used as parameter to a {@link org.jfree.chart.axis.DateAxis} to define the values that this axis
 * supports. This class implements a timeline formed by segments of equal length (ex. days, hours, minutes) where some
 * segments can be included in the timeline and others excluded. Therefore timelines like "working days" or "working
 * hours" can be created where non-working days or non-working hours respectively can be removed from the timeline, and
 * therefore from the axis. This creates a smooth plot with equal separation between all included segments.
 * <P>
 * Because Timelines were created mainly for Date related axis, values are represented as longs instead of doubles. In
 * this case, the domain value is just the number of milliseconds since January 1, 1970, 00:00:00 GMT as defined by the
 * getTime() method of {@link java.util.Date}.
 * <P>
 * In this class, a segment is defined as a unit of time of fixed length. Examples of segments are: days, hours,
 * minutes, etc. The size of a segment is defined as the number of milliseconds in the segment. Some useful segment
 * sizes are defined as constants in this class: DAY_SEGMENT_SIZE, HOUR_SEGMENT_SIZE, FIFTEEN_MINUTE_SEGMENT_SIZE and
 * MINUTE_SEGMENT_SIZE.
 * <P>
 * Segments are group together to form a Segment Group. Each Segment Group will contain a number of Segments included
 * and a number of Segments excluded. This Segment Group structure will repeat for the whole timeline.
 * <P>
 * For example, a working days SegmentedTimeline would be formed by a group of 7 daily segments, where there are 5
 * included (Monday through Friday) and 2 excluded (Saturday and Sunday) segments.
 * <P>
 * Following is a diagram that explains the major attributes that define a segment. Each box is one segment and must be
 * of fixed length (ms, second, hour, day, etc).
 * <p>
 * 
 * <pre>
 * start time
 ...
 * </ul>
 * <p>
 * Exception Segments are allowed. These exception segments are defined as segments that would have been in the included
 * segments of the Segment Group, but should be excluded for special reasons. In the previous working days
 * SegmentedTimeline example, holidays would be considered exceptions.
 * <P>
 * Additionally the <code>startTime</code>, or start of the first Segment of the smallest segment group needs to be
 * defined. This startTime could be relative to January 1, 1970, 00:00:00 GMT or any other date. This creates a point of
 * reference to start counting Segment Groups. For example, for the working days SegmentedTimeline, the
 * <code>startTime</code> could be 00:00:00 GMT of the first Monday after January 1, 1970. In this class, the constant
 * FIRST_MONDAY_AFTER_1900 refers to a reference point of the first Monday of the last century.
 * <p>
 * A SegmentedTimeline can include a baseTimeline. This combination of timelines allows the creation of more complex
 * timelines. For example, in order to implement a SegmentedTimeline for an intraday stock trading application, where
 * the trading period is defined as 9:00 AM through 4:00 PM Monday through Friday, two SegmentedTimelines are used. The
 * first one (the baseTimeline) would be a working day SegmentedTimeline (daily timeline Monday through Friday). On top
 * of this baseTimeline, a second one is defined that maps the 9:00 AM to 4:00 PM period. Because the baseTimeline
 * defines a timeline of Monday through Friday, the resulting (combined) timeline will expose the period 9:00 AM through
 * 4:00 PM only on Monday through Friday, and will remove all other intermediate intervals.
 * <P>
 * Two factory methods newMondayThroughFridayTimeline() and newFifteenMinuteTimeline() are provided as examples to
 * create special SegmentedTimelines.
 *
...
public class SegmentedTimeline implements Timeline, Cloneable, Serializable {
    ...
    @Deprecated
    /**
     * Utility constant that defines the startTime as the first monday after 1/1/1970. This should be used when creating
     * a SegmentedTimeline for Monday through Friday. See static block below for calculation of this constant.
     *
     ...
     *
     * @deprecated As of 1.0.7. This field is initialised based on the default time zone, and doesn't take into account
     *             subsequent changes to the default.
     */
    public static TimeZone NO_DST_TIME_ZONE;

    @Deprecated
    /**
     * This is the default time zone where the application is running. See getTime() below where we make use of certain
     * transformations between times in the default time zone and the no-dst time zone used for our calculations.
     *
    ...
    /**
     * List of exception segments (exceptions segments that would otherwise be included based on the periodic (included,
     * excluded) grouping).
     */
    private List<Segment> exceptionSegments = new ArrayList<SegmentedTimeline.Segment>();

    /**
     * This base timeline is used to specify exceptions at a higher level. For example, if we are a intraday timeline
     * and want to exclude holidays, instead of having to exclude all intraday segments for the holiday, segments from
     * this base timeline can be excluded. This baseTimeline is always optional and is only a convenience method.
     * <p>
    ...
    /**
     * Constructs a new segmented timeline, optionaly using another segmented timeline as its base. This chaining of
     * SegmentedTimelines allows further segmentation into smaller timelines.
     *
     * If a base
     *
     * @param segmentSize      the size of a segment in ms. This time unit will be used to compute the included and
     *                         excluded segments of the timeline.
     ...
     * <P>
     * The <code>startTime</code> of the resulting timeline will be midnight of the first Monday after 1/1/1900.
     *
     ...
     * <P>
     * This timeline uses a segmentSize of FIFTEEN_MIN_SEGMENT_SIZE. The segment group is defined as 28 included
     * segments (9:00 AM through 4:00 PM) and 68 excluded segments (4:00 PM through 9:00 AM the next day).
     * <P>
     * In order to exclude Saturdays and Sundays it uses a baseTimeline that only includes Monday through Friday days.
     * <P>
     * The <code>startTime</code> of the resulting timeline will be 9:00 AM after the startTime of the baseTimeline.
     * This will correspond to 9:00 AM of the first Monday after 1/1/1900.
     *
     * @return A fully initialized SegmentedTimeline.
     */
    ...
    /**
     * Returns the number of segments in a segment group. This will be equal to segments included plus segments
     * excluded.
     *
    ...
    /**
     * Returns the size in milliseconds of a segment group. This will be equal to size of the segments included plus the
     * size of the segments excluded.
     *
    ...
    /**
     * Translates a value relative to the domain value (all Dates) into a value relative to the segmented timeline. The
     * values relative to the segmented timeline are all consecutives starting at zero at the startTime.
     *
        ...
        if (groupMilliseconds >= this.segmentsIncludedSize) {
            ...
            if (segment.inExceptionSegments()) {
                ...
                while ((p = binarySearchExceptionSegments(segment)) >= 0) {
                    segment = getSegment(millisecond = this.exceptionSegments.get(p).getSegmentEnd() + 1);
                }
...
//                }
    ...
    /**
     * Translates a date into a value relative to the segmented timeline. The values relative to the segmented timeline
    ...
    /**
     * Returns <code>true</code> if a range of values are contained in the timeline. This is implemented verifying that
     * all segments are in the range.
     *
    ...
    /**
     * Adds a segment as an exception. An exception segment is defined as a segment to exclude from what would otherwise
     * be considered a valid segment of the timeline. An exception segment can not be contained inside an already
     * excluded segment. If so, no action will occur (the proposed exception segment will be discarded).
     * <p>
     * The segment is identified by a domainValue into any part of the segment. Therefore the segmentStart <=
     * domainValue <= segmentEnd.
     *
    ...
    /**
     * Adds a segment range as an exception. An exception segment is defined as a segment to exclude from what would
     * otherwise be considered a valid segment of the timeline. An exception segment can not be contained inside an
     * already excluded segment. If so, no action will occur (the proposed exception segment will be discarded).
     * <p>
     * The segment range is identified by a domainValue that begins a valid segment and ends with a domainValue that
     * ends a valid segment. Therefore the range will contain all segments whose segmentStart <= domainValue and
     * segmentEnd <= toDomainValue.
     *
    ...
    /**
     * Adds a list of dates as segment exceptions. Each exception segment is defined as a segment to exclude from what
     * would otherwise be considered a valid segment of the timeline. An exception segment can not be contained inside
     * an already excluded segment. If so, no action will occur (the proposed exception segment will be discarded).
     * <p>
     * The segment is identified by a Date into any part of the segment.
     *
     * @param exceptionList List of Date objects that identify the segments to exclude.
     */
    public void addExceptions(List<Date> exceptionList) {
        for (Iterator<Date> iter = exceptionList.iterator(); iter.hasNext();) {
     ...
     * Adds a segment as an exception. An exception segment is defined as a segment to exclude from what would otherwise
     * be considered a valid segment of the timeline. An exception segment can not be contained inside an already
     * excluded segment. This is verified inside this method, and if so, no action will occur (the proposed exception
    ...
    /**
     * Adds a segment relative to the baseTimeline as an exception. Because a base segment is normally larger than our
     * segments, this may add one or more segment ranges to the exception list.
     * <p>
     * An exception segment is defined as a segment to exclude from what would otherwise be considered a valid segment
     * of the timeline. An exception segment can not be contained inside an already excluded segment. If so, no action
    ...
    /**
     * Adds a segment relative to the baseTimeline as an exception. An exception segment is defined as a segment to
     * exclude from what would otherwise be considered a valid segment of the timeline. An exception segment can not be
     * contained inside an already excluded segment. If so, no action will occure (the proposed exception segment will
     * be discarded).
     * <p>
     * The segment is identified by a domainValue into any part of the segment. Therefore the segmentStart <=
     * domainValue <= segmentEnd.
    ...
    /**
     * Adds all excluded segments from the BaseTimeline as exceptions to our timeline. This allows us to combine two
     * timelines for more complex calculations.
     *
    ...
    public long getExceptionSegmentCount(long fromMillisecond, long toMillisecond) {
        ...
        int n = 0;
        for (Iterator<Segment> iter = this.exceptionSegments.iterator(); iter.hasNext();) {
    ...
    /**
     * Returns a segment that contains a domainValue. If the domainValue is not contained in the timeline (because it is
     * not contained in the baseTimeline), a Segment that contains <code>index + segmentSize*m</code> will be returned
    ...
    /**
     * Returns a segment that contains a date. For accurate calculations, the calendar should use TIME_ZONE for its
     * calculation (or any other similar time zone).
     *
     * If the date is not contained in the timeline (because it is not contained in the baseTimeline), a Segment that
     * contains <code>date + segmentSize*m</code> will be returned for the smallest <code>m</code> possible.
     *
    ...
    /**
     * Preforms a binary serach in the exceptionSegments sorted array. This array can contain Segments or SegmentRange
     * objects.
     *
     * @param segment the key to be searched for.
     *
     * @return index of the search segment, if it is contained in the list; otherwise,
     *         <tt>(-(<i>insertion point</i>) - 1)</tt>. The <i>insertion point</i> is defined as the point at which the
     *         segment would be inserted into the list: the index of the first element greater than the key, or
     *         <tt>list.size()</tt>, if all elements in the list are less than the specified segment. Note that this
    ...
    private int binarySearchExceptionSegments(Segment segment) {
        ...
        while (low <= high) {
            int mid = (low + high) / 2;
            Segment midSegment = this.exceptionSegments.get(mid);
    ...
    /**
     * Special method that handles conversion between the Default Time Zone and a UTC time zone with no DST. This is
     * needed so all days have the same size. This method is the prefered way of converting a Data into milliseconds for
     * usage in this class.
     *
    ...
    /**
     * Internal class to represent a valid segment for this timeline. A segment is valid on a timeline if it is part of
     * its included, excluded or exception segments.
     * <p>
    ...
    public class Segment implements Comparable<Segment>, Cloneable, Serializable {
        ...
        private static final long serialVersionUID = -5136212208010413322L;
         ...
         *
         * @param segment The other Segment to compare with
         *
        ...
        /**
         * Calculate the segment number relative to the segment group. This will be a number between 0 and
         * segmentsGroup-1. This value is calculated from the segmentNumber. Special care is taken for negative
         * segmentNumbers.
         *
         * @return The segment number.
         */
        ...
        /**
         * Returns true if we are an exception segment. This is implemented via a binary search on the exceptionSegments
         * sorted list.
         *
         * If the segment is not listed as an exception in our list and we have a baseTimeline, a check is performed to
         * see if the segment is inside an excluded segment from our base. If so, it is also considered an exception.
         *
    ...
    /**
     * Private internal class to represent a range of segments. This class is mainly used to store in one object a range
     * of exception segments. This optimizes certain timelines that use a small segment size (like an intraday timeline)
     * allowing them to express a day exception as one SegmentRange instead of multi Segments.
     */
    protected class SegmentRange extends Segment {

        private static final long serialVersionUID = -8690001960037633495L;
...
//            }
    ...
    protected class BaseTimelineSegmentRange extends SegmentRange {
        ...
        private static final long serialVersionUID = -5771733450436187002L;
